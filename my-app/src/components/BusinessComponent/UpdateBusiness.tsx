...
const UpdateBusiness: React.FC<IProps> = ({businessId}) => {
    ...
    const handleSubmit = async () => {
        ...
        axios.put(`http://localhost:8080/api/business/${businessId}`,{
              name: name,
              country: country,
              city: city,
              state: state,
          ...
          axios.put(`http://localhost:8080/api/graph/business/${businessId}`,{
              name: name
          }).then(
            function(response) {
                response.status === 200 ? setOpen(true) : setOpen(false);
